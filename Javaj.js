...
function Usermain() {
  ...
  function chooseDishes() {
    ...
    do {
      ...
      for (const word of words) {
        ...
        if (!found) {
          console.log(`Товар з ID ${word} не знайдений у меню , спробуйте ще раз :.`);
        }
      }
  ...
  function enterPersonalInfo() {
    const userInputName = prompt("Введіть ваше прізвище та ім'я через пробіл: ");
    const userInputemail = prompt("Введіть вашу електронну пошту: ");
    const userInputadress = prompt("Введіть вашу адресу доставки: ");
    ...
    do {
      userInputNumber = prompt("Уведіть ваш номер, типу (+380123456789): ");
...
function adminMain(){
  ...
  function changeOrderStatus() {
    const newStatus = prompt("Обновіть статус замовлення: 1 -- замовлення готується, 2 -- замовлення в дорозі, 3 -- замовлення доставлено, 4 -- схоже щось пішло не туди, \n5 -- нічого не міняти: ");
    ...
    do {
      ...
      if (newStatus == '4') {
        order.status = 'схоже щось пішло не туди';
      }
    } while (newStatus !== '5');
